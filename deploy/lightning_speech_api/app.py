"""FastAPI service for running Whisper and MMS transcription on GPU hosts."""
...
def _normalized_backend(value: str | None) -> str:
    ...
    if backend not in {"whisper", "mms"}:
        raise HTTPException(status_code=400, detail="backend must be 'whisper' or 'mms'.")
    return backend
...
def _transcribe(
    *,
    ...
    except (MMSDependencyError, WhisperDependencyError) as exc:
