...
_COMBINED_SOAP_ADDENDUM = (
    ...
    "[2-3 bullet TL;DR]\n\n"
    "S:\n"
    "Use these sub-labels, each on its own line; include a label only when there is content for it:\n"
    "CC: <chief complaint in one clinical English sentence>\n"
    "HPI: <history of present illness as a flowing clinical narrative>\n"
    ...
    "Current Medications: <one per line; write [dose not stated] when the dose is missing>\n"
    "Allergies: <drug allergies; write NKA if the doctor confirmed none>\n"
    "ROS: <review of systems not already covered in HPI, only if stated>\n\n"
    "O:\n"
    "Vitals: <BP / HR / RR / T / SpO2 / Wt / Ht / BMI / Glucose — only values the doctor stated; write [value not stated] where a vital is referenced but no number is given>\n"
    "Examination: <general appearance first, then system findings as indented bullets>\n"
    "Investigations: <labs, imaging, ECG — only values the doctor stated>\n\n"
    "A:\n<numbered problem list with impression/assessment>\n\n"
    "P:\n<numbered to match the assessment; under each number group Medications "
    "(Drug Dose Route Frequency x Duration), Investigations, Referrals, and Follow-up>\n\n"
